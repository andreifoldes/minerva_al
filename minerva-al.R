...
probe_memory_resonance <- function (probe, memory, cue_features, model = "Minerva AL", iteration){
  if(is.null(memory)) { # Empty memory
    echo <- runif(length(probe), -0.001, 0.001) # First trial is noise (p. 65, Jamieson, Crump, & Hannah, 2012)
    normalized_echo <- echo / max(abs(echo)) # Eq. 4, Jamieson, Crump, & Hannah (2012)
    return(normalized_echo)
    ...
    # calculate activation
    activation <- similarity^iteration                          # Eq. 2, Jamieson, Crump, & Hannah (2012)
    ...
    normalized_echo <- echo / max(abs(echo))                 # Eq. 4, Jamieson, Crump, & Hannah (2012)
    return(list(normalized_echo=normalized_echo, similarity=similarity))
  }
}
